...
var fromPrivate = function fromPrivate(privateKey) {
  ...
  var pubKeyBytes = secp256k1.getPublicKey(buffer, false);
  var publicKey = "0x" + Buffer.from(pubKeyBytes).toString('hex').slice(2);
...
var makeSigner = function makeSigner(addToV) {
  return function (hash, privateKey) {
    var sig = secp256k1.sign(Buffer.from(hash.slice(2), "hex"), Buffer.from(privateKey.slice(2), "hex"), {
      lowS: true
    });
    return encodeSignature([Nat.fromString(Bytes.fromNumber(addToV + sig.recovery)), Bytes.pad(32, Bytes.fromNat("0x" + sig.r.toString(16))), Bytes.pad(32, Bytes.fromNat("0x" + sig.s.toString(16)))]);
  };
};
...
var recover = function recover(hash, signature) {
  ...
  var recoveryBit = vrs.v < 2 ? vrs.v : 1 - (vrs.v % 2);
  var sig = secp256k1.Signature.fromCompact(Buffer.from(vrs.r + vrs.s, 'hex'));
  var point = sig.addRecoveryBit(recoveryBit).recoverPublicKey(Buffer.from(hash.slice(2), "hex"));
  var publicKey = "0x" + Buffer.from(point.toRawBytes(false)).toString('hex').slice(2);
  var publicHash = keccak256(publicKey);
  var address = toChecksum("0x" + publicHash.slice(-40));
  return address;
};
